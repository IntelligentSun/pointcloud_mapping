...
#include "pointcloud_mapper/loop_closure.h"
...
bool LaserLoopClosure::LoadParameters(const ros::NodeHandle& n) {
  ...
  n.param<bool>("check_for_loop_closures", check_for_loop_closures_, true);
  ...
  n.getParam("relinearize_skip", relinearize_skip);
  n.getParam("relinearize_threshold", relinearize_threshold);
  ...
  // are fast.
  n.param<double>("translation_threshold", translation_threshold_, 0.5);
  ...
  n.param<double>("proximity_threshold", proximity_threshold_, 1.0);
  ...
  n.param<double>("max_tolerable_fitness", max_tolerable_fitness_, 0.15);
  // Don't attempt loop closures with poses in the graph that were collected within
  // the last 'skip_recent_poses' poses.
  n.param<int>("skip_recent_poses", skip_recent_poses_, 40);
  ...
  n.param<int>("poses_before_reclosing", poses_before_reclosing_, 40);

  // Load ICP parameters.
  // Load algorithm parameters.
  // Stop ICP if the transformation from the last iteration was this small.
  n.param<double>("icp/tf_epsilon", icp_tf_epsilon_, 0.0000000001);
  // During ICP, two points won't be considered a correspondence if they are at
  // least this far from one another.
  n.param<double>("icp/corr_dist", icp_corr_dist_, 2.0);
  // Iterate ICP this many times.
  n.param<int>("icp/iterations", icp_iterations_, 10);
  ...
  n.getParam("init/position_sigma/x", sigma_x);
  n.getParam("init/position_sigma/y", sigma_y);
  n.getParam("init/position_sigma/z", sigma_z);
  n.getParam("init/orientation_sigma/roll", sigma_roll);
  n.getParam("init/orientation_sigma/pitch", sigma_pitch);
  n.getParam("init/orientation_sigma/yaw", sigma_yaw);
...
//  filter_.Filter(scan1, scan1);
//  filter_.Filter(scan2, scan2);
