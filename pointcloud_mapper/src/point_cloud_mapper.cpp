...
#include "pointcloud_mapper/point_cloud_mapper.h"
...
bool PointCloudMapper::LoadParameters(const ros::NodeHandle& n) {
  ...
  private_nh.param<std::string>("frame_id/fixed",fixed_frame_id_,"world");
  map_data_->header.frame_id = fixed_frame_id_;

  // Load map parameters.
  private_nh.param<double>("map/octree_resolution",octree_resolution_,0.01);
...
bool PointCloudMapper::RegisterCallbacks(const ros::NodeHandle& n) {
  ...
  incremental_map_pub_ = nl.advertise<PointCloud>("octree_map_updates", 10, false);
